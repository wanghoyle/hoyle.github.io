...
\begin{equation}
  f\left( \bf{m} \right) = \sum\limits_{i = 1}^m {\left( \frac{ G(\bf{m})_i - \bf{d}_i }{ \sigma _i } \right)}^2 \label{1}
\end{equation}

定义标量函数
\begin{equation}
  f_i\left( \bf{m} \right) = \frac{ G(\bf{m})_i - \bf{d}_i }{ \sigma _i },i = 1,2, \ldots ,m\label{2}
\end{equation}

以及向量函数
\begin{equation}
  \bf{F}\left( {\bf{m}} \right) = [ {f_1 \left( {\bf{m}} \right)} \ldots {f_m\left( {\bf{m}} \right)} ]^T\label{3}
\end{equation}

因此
\begin{equation}
  f\left( {\bf{m}} \right) = \sum\limits_{i = 1}^m {f_i \left( \bf{m} \right)^2} = \sum\limits_{i = 1}^m {\left\|\left\| \bf{F} \left( \bf{m} \right) \right\|\right\|_2^2}\label{4}
\end{equation}

$$ f\left( {\bf{m}} \right)$$ 的梯度可以写为其中每一项的梯度和，然后写成以下矩阵形式
\begin{equation}
  \nabla f\left( \bf{m} \right) = \sum\limits_{i = 1}^m {\nabla \left( {f_i {\left( \bf{m} \right)}^2} \right)} = 2\bf{J}{\left( \bf{m} \right)^T}\bf{F}\left( \bf{m} \right)\label{5}
\end{equation}

其中 $$ \bf{J}\left( \bf{m} \right)$$ 是 $$ \bf{F}\left( \bf{m} \right)$$ 的雅克比矩阵。与此类似，我们用 $$ f_i\left( \bf{m} \right)$$ 项得到了 $$ f\left( \bf{m} \right)$$ 的海森矩阵。因此
\begin{equation}
  \bf{H}\left( f \left( \bf{m} \right) \right) = 2\bf{J}{\left( \bf{m} \right)^T} \bf{J} \left( \bf{m} \right) + \bf{Q} \left( \bf{m} \right)\label{6}
\end{equation}

其中
\begin{equation}
  \bf{Q}\left( \bf{m} \right) = 2\sum\limits_{i = 1}^m {f_i \left( \bf{m} \right) \bf{H} \left( {f_i \left( \bf{m} \right)} \right)}\label{7}
\end{equation}

在**高斯-牛顿（GN）法**中，我们忽略(\ref{6})中的 $$ \bf{Q} \left( \bf{m} \right)$$ 项来近似海森矩阵
\begin{equation}{\bf{H}}\left( {f\left( {\bf{m}} \right)} \right) \approx 2{\bf{J}}{\left( {\bf{m}} \right)^T}{\bf{J}}\left( {\bf{m}} \right)\label{8}\end{equation}
...
用梯度 $$ \nabla f\left( \bf{m} \right)$$ (\ref{5}) 和近似的海森矩阵 $$ \bf{H} \left( {f\left( \bf{m} \right)} \right)$$ (\ref{8}) 来实现牛顿法最小化 $$ f\left( \bf{m} \right)$$，方程两边同时除以2，我们得到
\begin{equation}
  \bf{J} {\left( \bf{m}^k \right)^T} \bf{J} \left( \bf{m}^k \right)\Delta \bf{m} = - \bf{J} {\left( \bf{m}^k \right)^T} \bf{F}\left( \bf{m}^k \right)\label{9}
\end{equation}
...
在**利文贝格-麦夸特（LM）法**中，GN法模型更新方程 (\ref{9}) 被修改为
\begin{equation}
  \left( \bf{J} \left( \bf{m}^k \right)^T \bf{J} \left( \bf{m}^k \right) + \lambda \bf{I} \right)\Delta \bf{m} = - \bf{J} {\left( \bf{m}^k \right)^T} \bf{F}\left( \bf{m}^k \right)\label{10}
\end{equation}

其中大于零的参数 $$ \lambda$$ 在迭代过程中是自适应的以保证收敛。进行如此修改的一个重要原因是为了保证 (\ref{10}) 式左边的矩阵是非奇异的。因为此时系统方程中的矩阵是对称且正定的，我们可以使用 Cholesky 分解来有效地求解系统模型的更新步长 $$ \delta \bf{m}$$。
